...
// we use const because we don't want to modify at all while reading
...
// generates a random value 0-100, while running, we lock our shared resource (the distribution generator value)
// read, assign it to lightData, then delay for a second, continuously assigning the new light value to lightData each second
void LightSensor::run() {
    std::default_random_engine generator;
    std::uniform_int_distribution<int> distribution(0, 100);

    while (running) {
        { //another set of brackets for the scope of the lock
